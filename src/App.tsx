	...
	useEffect(() => {
		const keyboardHandler = (e: KeyboardEvent) => {
			...
			if (
				...
				switch (e.code) {
					case "ArrowRight":
						return addPoint("secondPlayer");

					case "ArrowLeft":
						return addPoint("firstPlayer");

					case "Space":
						e.preventDefault();
						return setFightStage(STAGES.PAUSE);
				}
			}
			if (fightState.stage === STAGES.PAUSE && e.code === "Space") {
				e.preventDefault();
				return setFightStage(fightState.prevStage);
			}
		};
		document.addEventListener("keydown", keyboardHandler);
		return () => document.removeEventListener("keydown", keyboardHandler);
	}, [fightState, addPoint, setFightStage]);
